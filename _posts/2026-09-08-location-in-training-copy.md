...
- **Use the merge fields you already know.** Type `{{location}}` in Course screens and Messages the way you already type `{{name}}` and `{{company}}`. Each Trainee sees their Location filled in.
  - **Example:** "Welcome to the team at `{{location}}`" becomes "Welcome to the team at Downtown" for a Trainee at Downtown. No need to create a separate version for each Location.
